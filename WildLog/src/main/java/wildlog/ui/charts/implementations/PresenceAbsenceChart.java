...
public class PresenceAbsenceChart extends AbstractChart<Sighting> {
    ...
    public PresenceAbsenceChart(List<Sighting> inLstData,JLabel inChartDescLabel, ChartsBaseDialog inReportsBaseDialog) {
...
// TODO: Ek weet nie of dit 'n probleem is of nie, maar hierdie werk op kalender dae, nie day-night-cycles nie...
    ...
    private Chart createPresenceAbsenceChart(List<Sighting> inSightings) {
        ...
        Map<String, Map<Integer, ChartDataWrapper>> mapGroupedData = new HashMap<>();
        ...
        for (Sighting sighting : inSightings) {
            ...
            Map<Integer, ChartDataWrapper> mapIntervalData = mapGroupedData.get(key);
            if (mapIntervalData == null) {
                mapIntervalData = new HashMap<>();
                mapGroupedData.put(key, mapIntervalData);
                // Add the current interval
                mapIntervalData.put(intervalCount, new ChartDataWrapper(key, Integer.toString(intervalCount), 0));
            }
            ...
            ChartDataWrapper dataWrapper = mapIntervalData.get(intervalCount);
            if (dataWrapper == null) {
                dataWrapper = new ChartDataWrapper(key, Integer.toString(intervalCount), 0);
        ...
        for (String key : lstKeys) {
            ...
            for (int i = 0; i <= intervalCount; i++) {
                ChartDataWrapper dataWrapper = mapIntervalData.get(i);
                if (dataWrapper == null) {
                    dataWrapper = new ChartDataWrapper(key, Integer.toString(i), 0);
        ...
        for (String key : lstKeys) {
            ...
            for (Integer intervalKey : lstIntervalKeys) {
                ChartDataWrapper dataWrapper = mapIntervalData.get(intervalKey);
                int size;

...
public class TextReports extends AbstractChart<Sighting> {
    ...
    public TextReports(List<Sighting> inLstData, JLabel inChartDescLabel, ChartsBaseDialog inReportsBaseDialog) {
    ...
    private Parent createDataReport(List<Sighting> inSightings) {
        ...
        Map<String, ChartDataWrapper> mapMinMax = new HashMap<>(setElementNames.size() * setLocationNames.size());
        for (Sighting sighting : inSightings) {
            ChartDataWrapper countWrapper = mapMinMax.get(sighting.getCachedElementName(reportsBaseDialog.getOptionName()));
            if (countWrapper == null) {
                countWrapper = new ChartDataWrapper(sighting.getCachedElementName(reportsBaseDialog.getOptionName()), null, 0);
        ...
        int ave = 0;
        int max = 0;
        for (ChartDataWrapper dataWrapper : mapMinMax.values()) {

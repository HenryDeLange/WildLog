...
public class RelationshipsChart extends AbstractChart<Sighting> {
    ...
    public RelationshipsChart(List<Sighting> inLstData, JLabel inChartDescLabel, ChartsBaseDialog inReportsBaseDialog) {
...
// TODO: Sit 'n beskrywing onder aan die chart self (en dalk in die titel) wat verduidelik hoe die opsies dinge beinvloed
    ...
    private Chart createElementRelationshipChart(List<Sighting> inSightings) {
        ...
        for (Sighting sighting1 : inSightings) {
            ...
            for (Sighting sighting2 : inSightings) {
                if (isRelated(sighting1, sighting2)) {
                    ...
                    ChartDataWrapper dataWrapper = mapChartData.get(groupName);
                    if (dataWrapper == null) {
                        BubbleInfo bubbleInfo = new BubbleInfo();
                        dataWrapper = new ChartDataWrapper(groupName, bubbleInfo, 0);
        ...
        for (String key : keys) {
            ...
            ChartDataWrapper dataWrapper = mapChartData.get(key);
        ...
        UtilsCharts.setupCategoryAxis(catAxis, mapChartData.size(), false);

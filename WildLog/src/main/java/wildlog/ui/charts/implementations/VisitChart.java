...
public class VisitChart extends AbstractChart<Sighting> {
...
// TODO: Klompie van hierdie charts sal beter werk as line (area) charts
    ...
    public VisitChart(List<Sighting> inLstData,JLabel inChartDescLabel, ChartsBaseDialog inReportsBaseDialog) {
    ...
    private Chart createPieChartSightingCount(List<Sighting> inSightings) {
        ...
        for (Sighting sighting : inSightings) {
            ChartDataWrapper dataWrapper = mapGroupedData.get(sighting.getCachedVisitName());
            if (dataWrapper == null) {
                mapGroupedData.put(sighting.getCachedVisitName(), new ChartDataWrapper("", "", 1));
            }
            else {
    ...
    private Chart createPieChartVisitTypes(List<Sighting> inSightings) {
        ...
        for (Sighting sighting : inSightings) {
            ...
            ChartDataWrapper dataWrapper = mapGroupedData.get(category);
            if (dataWrapper == null) {
                mapGroupedData.put(category, new ChartDataWrapper("", "", 1));
            }
            else {
                dataWrapper.increaseCount();
            }
    ...
    private Chart createBarChartDurationCombined(List<Sighting> inSightings) {
        ...
        for (Sighting sighting : inSightings) {
            if (!processedVisits.contains(sighting.getCachedVisitName())) {
                ...
                ChartDataWrapper dataWrapper = mapData.get(duration);
                if (dataWrapper == null) {
                    dataWrapper = new ChartDataWrapper(duration, "", 0);
    ...
    private Chart createBarChartSightingCount(List<Sighting> inSightings) {
        ...
        for (Sighting sighting : inSightings) {
            ChartDataWrapper dataWrapper = mapData.get(sighting.getCachedVisitName());
            if (dataWrapper == null) {
                dataWrapper = new ChartDataWrapper(sighting.getCachedVisitName(), null, 0);
                mapData.put(sighting.getCachedVisitName(), dataWrapper);
            }
            dataWrapper.increaseCount();
        }

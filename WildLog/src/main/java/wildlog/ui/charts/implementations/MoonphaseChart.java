...
public class MoonphaseChart extends AbstractChart<Sighting> {
    ...
    public MoonphaseChart(List<Sighting> inLstData, JLabel inChartDescLabel, ChartsBaseDialog inReportsBaseDialog) {
...
// TODO: Maak dalk twee opsies: een vir "Baseline at Night" en een vir "Baseline at Day"
    ...
    private Chart createBarChart(List<Sighting> inSightings, boolean inIsForAllObservations) {
        ...
        for (Sighting sighting : inSightings) {
            ...
            Map<String, ChartDataWrapper> mapChartData = mapChartDataGroupedForSeries.get(temp + getDetailsString(sighting));
        ...
        // Add the baseline
        if (showBaseline) {
            Map<String, ChartDataWrapper> mapChartData = new HashMap<>();
        ...
        for (String key : keys) {
            ...
            Map<String, ChartDataWrapper> mapChartData = mapChartDataGroupedForSeries.get(key);
            int total = 0;
        ...
        UtilsCharts.setupCategoryAxis(catAxis, 3, true);
        ...
        UtilsCharts.setupChartTooltips(chart, true, false, true);
        return chart;
    }
    
    private Chart createPieChart(List<Sighting> inSightings) {
        Map<String, ChartDataWrapper> mapChartData = new HashMap<>(5);
        for (Sighting sighting : inSightings) {
    ...
    private Chart createLineChart(List<Sighting> inSightings, boolean inIsForAllObservations) {
        ...
        for (Sighting sighting : inSightings) {
            ...
            ChartDataWrapper dataWrapper = mapInitialCountedData.get(temp + getDetailsString(sighting) + "-" + getMoonIntervalPercentage(sighting.getMoonPhase()));
            if (dataWrapper == null) {
        ...
        if (showBaseline) {
            ...
            for (VisitDates visitDates : mapVisitDates.values()) {
                if (visitDates.startDate != null && visitDates.endDate != null) {
                    for (LocalDate day = visitDates.startDate; day.isBefore(visitDates.endDate); day = day.plusDays(1)) {
                        int moonphase = AstroCalculator.getMoonPhase(Date.from(day.atStartOfDay(ZoneId.systemDefault()).toInstant()));
                        ChartDataWrapper dataWrapper = mapInitialCountedData.get(BASELINE + "-" + getMoonIntervalPercentage(moonphase));
                        if (dataWrapper == null) {
                            dataWrapper = new ChartDataWrapper();
        ...
        UtilsCharts.setupNumberAxis(numAxis, false);
        CategoryAxis catAxis = new CategoryAxis();
        UtilsCharts.setupCategoryAxis(catAxis, 3, false);
    ...
    private void increaseCountInMap(Map<String, ChartDataWrapper> inMap, String inKey) {
        ChartDataWrapper dataWrapper = inMap.get(inKey);
        if (dataWrapper == null) {
            dataWrapper = new ChartDataWrapper(inKey, inKey, 0);

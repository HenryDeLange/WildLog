...
public class LocationChart extends AbstractChart<Sighting> {
    private enum ChartType {PIE_CHART_SIGHITNGS, BAR_CHART_SIGHITNGS, BAR_CHART_ELEMENTS};
    private ChartType chartType;

    
    public LocationChart(List<Sighting> inLstData,JLabel inChartDescLabel, ChartsBaseDialog inReportsBaseDialog) {
    ...
    private Chart createBarChartSightings(List<Sighting> inSightings) {
        ...
        for (Sighting sighting : inSightings) {
            ChartDataWrapper dataWrapper = mapData.get(sighting.getCachedLocationName());
            if (dataWrapper == null) {
                dataWrapper = new ChartDataWrapper(sighting.getCachedLocationName(), null, 0);
    ...
    private Chart createPieChartSightings(List<Sighting> inSightings) {
        ...
        for (Sighting sighting : inSightings) {
             ChartDataWrapper dataWrapper = mapGroupedData.get(sighting.getCachedLocationName());
            if (dataWrapper == null) {
                mapGroupedData.put(sighting.getCachedLocationName(), new ChartDataWrapper("", "", 1));
            }
            else {
                dataWrapper.increaseCount();
            }

...
public class SightingStatsChart extends AbstractChart<Sighting> {
...
//        [Weighted Average (Weighted Mean)] -> The more time a value is present the more relevant it becomes
    ...
    public SightingStatsChart(List<Sighting> inLstData, JLabel inChartDescLabel, ChartsBaseDialog inReportsBaseDialog) {
...
//        }
    ...
    private Chart createNumOfElementsPerSightingBarChart(List<Sighting> inSightings) {
        ...
        Map<String, ChartCountWrapper> mapElemNum = new HashMap<>();
        for (Sighting sighting : inSightings) {
            ChartCountWrapper countWrapper = mapElemNum.get(sighting.getCachedElementName(reportsBaseDialog.getOptionName()));
            if (countWrapper == null) {
                countWrapper = new ChartCountWrapper(0, 0, 0, 0);
        ...
        UtilsCharts.setupNumberAxis(numAxis, true);
        CategoryAxis catAxis = new CategoryAxis();
        UtilsCharts.setupCategoryAxis(catAxis, mapElemNum.size(), false);
    ...
    private Chart createSightingPerDayBarChart(List<Sighting> inSightings) {
        ...
        // Get the data structured
        Map<String, Map<LocalDate, ChartCountWrapper>> mapElemCount = new HashMap<>();
        for (Sighting sighting : inSightings) {
            LocalDateTime currentDate = UtilsTime.getLocalDateTimeFromDate(sighting.getDate());
            Map<LocalDate, ChartCountWrapper> mapCount = mapElemCount.get(sighting.getCachedElementName(reportsBaseDialog.getOptionName()));
            if (mapCount == null) {
            ...
            if (ActiveTime.DAY.equals(ActiveTime.getFromActiveTimeSpecific(sighting.getTimeOfDay()))) {
                ...
                ChartCountWrapper countWrapper = mapCount.get(currentDate.toLocalDate());
                if (countWrapper == null) {
                    countWrapper = new ChartCountWrapper();
            ...
            if (ActiveTime.NIGHT.equals(ActiveTime.getFromActiveTimeSpecific(sighting.getTimeOfDay()))) {
                ...
                ChartCountWrapper countWrapper = mapCount.get(adjustedDate);
                if (countWrapper == null) {
                    countWrapper = new ChartCountWrapper();
        ...
        for (String key : keys) {
            ...
            Map<LocalDate, ChartCountWrapper> mapCount = mapElemCount.get(key);
        ...
        UtilsCharts.setupNumberAxis(numAxis, true);
        CategoryAxis catAxis = new CategoryAxis();
        UtilsCharts.setupCategoryAxis(catAxis, mapElemCount.size(), false);
        BarChart<Number, String> chart = new BarChart<Number, String>(numAxis, catAxis, lstFinalChartDataSeries);
        chart.getStyleClass().add("wl-bar-single-color");
        chart.setLegendVisible(false);
...
//    }
    ...
    private Chart createFirstSightingBarChart(List<Sighting> inSightings) {
        ...
        // Get the data structured
        Map<String, ChartCountWrapper> mapElemDate = new HashMap<>();
        for (Sighting sighting : inSightings) {
            LocalDateTime currentDate = UtilsTime.getLocalDateTimeFromDate(sighting.getDate());
            LocalDate adjustedDate = currentDate.toLocalDate();
            if (ActiveTime.NIGHT.equals(ActiveTime.getFromActiveTimeSpecific(sighting.getTimeOfDay()))) {
            ...
            ChartCountWrapper wrapper = mapElemDate.get(sighting.getCachedElementName(reportsBaseDialog.getOptionName()));
            if (wrapper == null) {
                wrapper = new ChartCountWrapper();
        ...
        Collections.sort(keys);
        for (String key : keys) {
            ChartCountWrapper wrapper = mapElemDate.get(key);
            if (cmbOption.getSelectionModel().isSelected(0)) { // Min
        ...
        UtilsCharts.setupNumberAxis(numAxis, true);
        CategoryAxis catAxis = new CategoryAxis();
        UtilsCharts.setupCategoryAxis(catAxis, 0, false);
        BarChart<Number, String> chart = new BarChart<Number, String>(numAxis, catAxis, lstFinalChartDataSeries);
        chart.getStyleClass().add("wl-bar-single-color");
        chart.setLegendVisible(false);

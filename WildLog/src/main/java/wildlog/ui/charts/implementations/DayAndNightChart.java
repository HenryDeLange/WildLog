...
public class DayAndNightChart extends AbstractChart<Sighting> {
    private enum ChartType {PIE_CHART, LINE_CHART, STACKED_LINE_CHART};
    private ChartType chartType = ChartType.PIE_CHART;

    
    public DayAndNightChart(List<Sighting> inLstData, JLabel inChartDescLabel, ChartsBaseDialog inReportsBaseDialog) {
        super("Day and Night Cycle Charts", inLstData, inChartDescLabel, inReportsBaseDialog);
    ...
    private Chart createPieChart(List<Sighting> inSightings) {
        Map<String, ChartDataWrapper> mapGroupedData = new HashMap<>(4);
        for (Sighting sighting : inSightings) {
            ChartDataWrapper dataWrapper = mapGroupedData.get(ActiveTime.getFromActiveTimeSpecific(sighting.getTimeOfDay()).toString());
            if (dataWrapper == null) {
                mapGroupedData.put(ActiveTime.getFromActiveTimeSpecific(sighting.getTimeOfDay()).toString(), new ChartDataWrapper("", "", 1));
            }
            else {
        ...
        for (String key : keys) {
            ChartDataWrapper dataWrapper = mapGroupedData.get(key);
            if (dataWrapper != null) {
...
// FIXME: Kry 'n manier om die lee data se labels te hide want dit lyk simpel (vir nou is die if hier onder OK)
    ...
    private Chart createLineChartForAll(List<Sighting> inSightings) {
        ...
        Map<String, Map<Long, ChartDataWrapper>> mapGroupedData = new HashMap<>(4);
        Map<String, ChartDataWrapper> mapCategoryCounter = new HashMap<>(4);
        for (Sighting sighting : inSightings) {
            String categoryKey = ActiveTime.getFromActiveTimeSpecific(sighting.getTimeOfDay()).toString();
            Map<Long, ChartDataWrapper> mapChartData = mapGroupedData.get(categoryKey);
            if (mapChartData == null) {
                mapChartData = new LinkedHashMap<>();
                mapGroupedData.put(categoryKey, mapChartData);
            }
            ChartDataWrapper categoryDataWrapper = mapCategoryCounter.get(categoryKey);
            if (categoryDataWrapper == null) {
                categoryDataWrapper = new ChartDataWrapper(null, null, 0);
                mapCategoryCounter.put(categoryKey, categoryDataWrapper);
            }
            categoryDataWrapper.increaseCount();
            ChartDataWrapper dataWrapper = mapChartData.get(sighting.getDate().getTime());
            if (dataWrapper == null) {
                dataWrapper = new ChartDataWrapper(null, null, categoryDataWrapper.count);
        ...
        if (!inSightings.isEmpty()) {
            ...
            Collections.sort(keys);
            for (String key : keys) {
                Map<Long, ChartDataWrapper> mapChartData = mapGroupedData.get(key);
                if (mapChartData != null) {
                    ...
                    for (long time : mapChartData.keySet()) {
                        ChartDataWrapper dataWrapper = mapChartData.get(time);
        ...
        chart.setTitle("Number of Observations per Day, Night and Twilight period");
        UtilsCharts.setupChartTooltips(chart, true, true);
        return chart;
    }
